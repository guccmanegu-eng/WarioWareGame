...
var Bomb_collected = 0 # just keeping track of Bomb collected
...
func _process(delta: float) -> void: # running every frame brochacho
	...
	if Bomb_collected == 3: # the double equals is just an argument asking if it's the same, with "=" it'll give an error
...
func Bomb_collect() -> void: # cool function that you connect to those Bombs
